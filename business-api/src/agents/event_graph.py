"""Bounded event-graph helpers for task and agent history."""
...
def build_graph_node(
    ...
    *,
    ...
    """Build a schema-safe graph node."""
    return {
        "id": node_id,
        "type": node_type,
...
def build_graph_edge(
    ...
    *,
    ...
    """Build a schema-safe graph edge."""
    return {
        "source": source,
        "target": target,
        "type": relation,
...
def normalize_event_graph(graph: dict[str, Any] | None) -> dict[str, Any]:
    """Return a bounded event graph structure."""
...
def append_graph_event(
    ...
    *,
    ...
    """Append a node/edge pair to a bounded event graph."""

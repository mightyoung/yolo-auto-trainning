"""Bounded output spool helpers for agent-side long-running work."""
...
_ROOT_DIR = Path(tempfile.gettempdir()) / "yolo-auto-training" / "business-api" / "task-output"
...
class TaskOutputSpool:
    """File-backed spool that keeps long-running output bounded."""
...
def append_agent_output(
    ...
    *,
    ...
    """Append output to the agent spool and persist the bounded snapshot."""
    spool = get_task_output_spool(task_id)
    snapshot = spool.append(text, summary=summary)
    if redis_client is not None:
        redis_client.hset(
            f"agent:{task_id}",
            mapping={
                "output_path": snapshot["output_path"],
                "output_offset": str(snapshot["output_offset"]),
                "output_summary": snapshot["output_summary"] or "",
                "output_capped": str(snapshot["output_capped"]).lower(),
            },
        )
    return snapshot
...
def apply_output_snapshot(record: dict[str, Any], snapshot: dict[str, Any]) -> dict[str, Any]:
    """Attach a bounded output snapshot to a task or agent record."""
    updated = dict(record)

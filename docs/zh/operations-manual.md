...
```
+------------------------------------------------------------------+
|                  Business API (端口 8000)                        |
|           认证 │ 数据集发现 │ CrewAI 智能体 │ 任务调度             |
+---------------------------+--------------------------------------+
                            | 内部 HTTP 调用
+---------------------------v--------------------------------------+
|                  Training API (端口 8001)                         |
|           YOLO 训练 │ HPO │ 模型导出 │ 平台期检测               |
+---------------------------+--------------------------------------+
                            |
         +------------------+-------------------+
         |                  |                   |
...
# 启动 Business API（终端 1）- 认证、数据集发现、CrewAI 智能体
uvicorn business-api.src.api.gateway:app --host 0.0.0.0 --port 8000

# 启动 Training API（终端 2）- GPU 训练、HPO、模型导出
